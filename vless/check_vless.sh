...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/serv00.hengda.us.kg/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/serv00.hengda.us.kg/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/serv00.hengda.us.kg/vless/config.json > ~/domains/serv00.hengda.us.kg/vless/config_tmp.json && mv ~/domains/serv00.hengda.us.kg/vless/config_tmp.json ~/domains/serv00.hengda.us.kg/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/serv00.hengda.us.kg/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}$serv00.hengda.us.kg${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/serv00.hengda.us.kg/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@serv00.hengda.us.kg:${port}?flow=&security=none&encryption=none&type=ws&host=serv00.hengda.us.kg&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless${NC}"
}
